    ...
    <div
        ...
        <div class="container mx-auto px-4 py-8">
            <div class="max-w-3xl mx-auto">
                ...
                @if (session('success'))
                    <div class="mb-4 rounded border border-green-200 bg-green-50 p-3 text-green-800">{{ session('success') }}
                    </div>
                @endif
                @if (session('warning'))
                    <div class="mb-4 rounded border border-yellow-200 bg-yellow-50 p-3 text-yellow-800">
                        {{ session('warning') }}</div>
                @endif
                ...
                <div class="mt-6">
                    ...
                    @if ($bgFile)
                        <div class="mb-3">
                            <img src="{{ asset($bgFile) }}" alt="background"
                                style="max-width:100%; height:auto; display:block;" />
                        </div>
                        <form action="{{ route('admin.background.destroy') }}" method="post"
                            onsubmit="return confirm('Hapus background lama?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="text-red-500 bg-white border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 dark:bg-zink-700 dark:hover:bg-red-500 dark:ring-red-400/20 dark:focus:bg-red-500">Hapus
                                Background</button>
                        </form>
                    @else
