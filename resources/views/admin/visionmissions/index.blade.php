...
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="flex items-center justify-between mb-4">
                            <a href="#!" data-modal-target="addVisiMisiModal" type="button"
                        ...
                        @if (isset($visionMissions) && $visionMissions->count())
                            <table id="rowBorder" class="w-full">
                                <thead>
                                    <tr>
                                        <th class="text-left p-2">No</th>
                                        <th class="text-left p-2">Visi</th>
                                        <th class="text-left p-2">Misi</th>
                                        <th class="text-left p-2">Dibuat oleh</th>
                                        <th class="text-left p-2">Diupdate oleh</th>
                                ...
                                <tbody class="divide-y divide-slate-200 dark:divide-zink-500">
                                    @foreach ($visionMissions as $v)
                                        <tr>
                                            ...
                                            <td class="p-2 align-top whitespace-normal max-w-[400px]">
                                                {{ Str::limit($v->vision, 150, '...') ?? '-' }}
                                            </td>

                                            {{-- Misi (jadi list kebawah) --}}
                                            <td class="p-2 align-top">
                                                @if (!empty($v->mission))
                                                    <a href="#!" data-modal-target="misiModal{{ $v->id }}"
                                                        type="button"
                                                        class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20 add-employee"><i
                                                            data-lucide="eye" class="inline-block size-4"></i> <span
                                                            class="align-middle">Lihat</span></a>

                                                    <div id="misiModal{{ $v->id }}" modal-center=""
                                                        ...
                                                        <div
                                                            ...
                                                            <div
                                                                ...
                                                                <h5 class="text-16" id="addEmployeeLabel">Misi</h5>
                                                            </div>
                                                            <div
                                                                ...
                                                                <div class="p-4 max-h-[70vh] overflow-y-auto">
                                                                    ...
                                                                    @if (count($missions))
                                                                        <ol
                                                                            class="list-decimal list-inside space-y-5 text-slate-700 dark:text-zink-100 leading-relaxed">
                                                                            @foreach ($missions as $m)
                                                                                <li
                                                                                    class="break-words whitespace-normal text-justify">
                                                                                    {{ trim($m) }}
                                                                                </li>
                                                                            @endforeach
                                                                        </ol>
                                                                    @else
                                                                        <p class="text-slate-500 italic">Belum ada misi yang
                                                                            ditambahkan.</p>
                                                                    @endif
                                                                </div>
                                                                <div class="flex justify-end gap-2 mt-4">
                                                                    <button type="reset" id="close-modal"
                                                                        data-modal-close="misiModal{{ $v->id }}"
                                            ...
                                            <td class="p-2">{{ $v->createdBy->name ?? '-' }}</td>
                                            <td class="p-2">{{ $v->updatedBy->name ?? '-' }}</td>
                                            <td class="p-2">
                                                <div class="flex gap-2">
                                                    <a href="#!"
                                                        data-modal-target="editVisiMisiModal{{ $v->id }}"
                                                    ...
                                                    <a href="#!" data-modal-target="deleteModal{{ $v->id }}"
                                        ...
                                        <div id="editVisiMisiModal{{ $v->id }}" modal-center=""
                                            class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
                                            <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
                                                <div
                                                    class="flex items-center justify-between p-4 border-b dark:border-zink-500">
                                                    <h5 class="text-16 font-medium" id="editVisiMisiLabel">Edit Visi & Misi
                                                    </h5>
                                                </div>
                                                ...
                                                <div
                                                    class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                                                    <form id="edit-form"
                                                        action="{{ route('admin.visionmissions.update', $v->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        ...
                                                        <div class="xl:col-span-6">
                                                            <label for="edit_s_menu_id"
                                                                class="inline-block mb-2 text-base font-medium">Menu</label>
                                                            <select
                        ...
                        placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                        ...
                                                        <div class="xl:col-span-6 mt-3">
                                                            <label for="edit_vision"
                                                                class="inline-block mb-2 text-base font-medium">Visi</label>
                                                            <input type="text" name="vision" id="edit_vision"
                                                                value="{{ old('vision', $v->vision) }}"
                                                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500
                        disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300
                        dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500
                        dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800
                        placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                        ...
                                                        <div class="xl:col-span-6 mt-3">
                                                            <label for="edit_mission"
                                                                class="inline-block mb-2 text-base font-medium">Misi</label>
                                                            <textarea rows="5" id="edit_mission" name="mission"
                                                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500
                        disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300
                        dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500
                        dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800
                        placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                                placeholder="Masukkan Misi" required>{{ old('mission', $v->mission) }}</textarea>
                                                        </div>

                                                        <div class="flex justify-end gap-2 mt-4">
                                                            <button type="button" id="close-edit-modal"
                                                                data-modal-close="editVisiMisiModal{{ $v->id }}"
                        ...
                        active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                        ...
                                        <div id="deleteModal{{ $v->id }}" modal-center=""
                                            class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
                                                    ...
                                                    <div class="mt-5 text-center">
                                                        ...
                                                        <div class="flex justify-center gap-2 mt-6">
                                                            <form class="delete-form"
                                                                action="{{ route('admin.visionmissions.destroy', $v->id) }}"
                                                                method="post">
                                                                @method('DELETE')
                                                                @csrf
                                                                <button type="reset"
                                                                    data-modal-close="deleteModal{{ $v->id }}"
                        ...
                        @else
                            ...
                            <div class="py-12 text-center">
                                ...
                                <h3 class="text-lg font-semibold mb-2">Belum ada Visi & Misi</h3>
                                <p class="text-sm text-slate-500 mb-4">Belum ada visi & misi yang dibuat. Klik tombol
                                    "Tambah
                                    Visi & Misi" untuk membuat visi misi baru.</p>
                            </div>
                        @endif
                    </div>
                </div><!--end card-->
                <div id="addVisiMisiModal" modal-center=""
                    class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
                    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
                        <div class="flex items-center justify-beSTEen p-4 border-b dark:border-zink-500">
                            <h5 class="text-16" id="addEmployeeLabel">Tambah Visi & Misi</h5>
                        </div>
                        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                            <form class="create-form" id="create-form" action="{{ route('admin.visionmissions.store') }}"
                                method="POST">
                                @csrf
                                <div class="xl:col-span-6">
                                    <label for="menus" class="inline-block mb-2 text-base font-medium">Menu</label>
                                    <select
                                ...
                                <div class="xl:col-span-6">
                                    <label for="slugInput" class="inline-block mb-2 text-base font-medium">Misi</label>
                                    <textarea rows="5" id="missionInput" name="mission"
                                        class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                        placeholder="Masukkan Misi" required></textarea>
                                </div>
                                <div class="flex justify-end gap-2 mt-4">
                                    <button type="reset" id="close-modal" data-modal-close="addVisiMisiModal"
                                        class="text-red-500 bg-white btn hover:text-red-500 hover:bg-red-100 focus:text-red-500 focus:bg-red-100 active:text-red-500 active:bg-red-100 dark:bg-zink-600 dark:hover:bg-red-500/10 dark:focus:bg-red-500/10 dark:active:bg-red-500/10">Cancel</button>
    ...
    <script>
        ...
        // Saat pertama kali fokus, isi otomatis dengan "1. " jika kosong
        ...
        // Tangani logika penomoran
        textarea.addEventListener('keydown', function(e) {
            const start = this.selectionStart;
            const end = this.selectionEnd;

            // Cegah menghapus "1. "
            if ((start <= 3 && ['Backspace', 'Delete'].includes(e.key)) ||
                (start < 3 && e.key.length === 1)) {
                e.preventDefault();
            }
            ...
            if (e.key === 'Enter') {
                ...
                // Cari baris terakhir yang punya angka di awal
                let lastNumber = 0;
                ...
                // Tambahkan baris baru dengan nomor berikut
                ...
                // Pindahkan kursor ke akhir nomor baru
                const newPosition = before.length + 4;
